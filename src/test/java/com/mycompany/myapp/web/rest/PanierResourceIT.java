...
class PanierResourceIT {
    ...
    public static Panier createEntity(EntityManager em) {
        Panier panier = new Panier().name(DEFAULT_NAME).price(DEFAULT_PRICE);
        // Add required entity
        Restorateur restorateur;
        if (TestUtil.findAll(em, Restorateur.class).isEmpty()) {
            restorateur = RestorateurResourceIT.createEntity(em);
            em.persist(restorateur);
            em.flush();
        } else {
            restorateur = TestUtil.findAll(em, Restorateur.class).get(0);
        }
        panier.setRestname(restorateur);
        // Add required entity
        Client client;
    ...
    public static Panier createUpdatedEntity(EntityManager em) {
        Panier panier = new Panier().name(UPDATED_NAME).price(UPDATED_PRICE);
        // Add required entity
        Restorateur restorateur;
        if (TestUtil.findAll(em, Restorateur.class).isEmpty()) {
            restorateur = RestorateurResourceIT.createUpdatedEntity(em);
            em.persist(restorateur);
            em.flush();
        } else {
            restorateur = TestUtil.findAll(em, Restorateur.class).get(0);
    ...
    void putExistingPanier() throws Exception {
        ...
        updatedPanier.name(UPDATED_NAME).price(UPDATED_PRICE);
    ...
    void fullUpdatePanierWithPatch() throws Exception {
        ...
        partialUpdatedPanier.name(UPDATED_NAME).price(UPDATED_PRICE);
